...
animated_object* bosses[MAX_BOSSES];


renderer* dr = NULL;
static int anim_id = 0;
static int displayNo = 100;
...
void rendering_init() {
	...
	for(i=0; i<MAX_BOSSES; i++)
	{
		...
		bosses[i] = entity_new(buffer, animated_object);

		animated_object_load_skeleton(bosses[i], asset_hndl_new(P("$CORANGE/rendering/imrod/imrod.skl")));
		...
		bosses[i]->animation[0] = asset_hndl_new(P("$CORANGE/rendering/imrod/imrod.ani"));
		bosses[i]->animation[1] = asset_hndl_new(P("$CORANGE/rendering/imrod/test/imrod-test.ani"));
...
void rendering_render() {
	...
	for(i=0; i<displayNo; i++)
		renderer_add(dr, render_object_animated( bosses[i] ));
